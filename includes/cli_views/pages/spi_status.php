...
<? foreach($channels as $channel => $value):?>
  <?=$GLOBALS['SPI_CHIP_SELECTS'][$chip_select]?> <?=str_pad($GLOBALS['SPI_CHANNELS'][$chip_select][$channel], 10, ' ',  STR_PAD_RIGHT)?> <?=str_pad($value, 4, ' ', STR_PAD_LEFT)?>  <?=graph_horizontal_bar($value, 0, 1023)?>
...
<? if(isset($spi_dac_values[$chip_select][$channel])):?>
  <?=$GLOBALS['SPI_CHIP_SELECTS'][$chip_select]?> <?=str_pad($channel_name, 10, ' ',  STR_PAD_RIGHT)?> <?=str_pad($spi_dac_values[$chip_select][$channel], 4, ' ', STR_PAD_LEFT)?>  <?=graph_horizontal_bar($spi_dac_values[$chip_select][$channel], 0, 4095)?>
